...
class LiveModuleRuntime(
    ...
    fun execute(input: String): ModuleExecution {
        ...
        for (step in current.parsed.steps) {
            val value = step.value.replace("${'$'}{input}", input)
            when (step.operation.lowercase()) {
                ...
                "replace_exact" -> output += "PROPOSAL_REQUIRED: live-module mutations must be approved through MutationCoordinator"
    ...
    @Synchronized
    fun applyPatch(transform: (String) -> String, action: AgentAction, input: String = ""): ModulePatchResult {
