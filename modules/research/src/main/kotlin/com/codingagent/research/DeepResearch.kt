...
class DurableDeepResearchProvider(
    ...
    override fun deepResearch(
        ...
        if (mode == ResearchMode.BROAD) {
            return PersonalResearchProvider(researchRoot, pageTimeoutMillis, connectionFactory, searchProvider)
                .deepResearch(query, targetSources, mode, onProgress, cancellation)
        }
        val effectiveTarget = targetSources.coerceIn(2, ResearchLimits.MAX_TARGET_SOURCES)
        ...
        val candidates = lanes.flatMap { lane ->
            if (cancellation.isCancelled()) return@flatMap emptyList()
            searchProvider.search(lane.query, ResearchLimits.MAX_SEARCH_RESULTS_PER_QUERY).hits
        ...
        if (candidates.size < effectiveTarget) {
            ...
            val fallback = focused.flatMap { fq ->
                if (cancellation.isCancelled()) return@flatMap emptyList()
                searchProvider.search(fq, ResearchLimits.MAX_SEARCH_RESULTS_PER_QUERY).hits
                    .filter { hit -> SourceQuality.isAcceptable(hit.url, hit.title, hit.excerpt) }
                    .filter { hit -> SourceQuality.hasQueryRelevance(queryTerms, hit.title, hit.excerpt, hit.url) }
                    .filter { hit -> hit.url.substringBefore('#').lowercase() !in alreadyLearned }
        ...
        diverse.forEachIndexed { index, candidate ->
            ...
            val fetched = runCatching {
                require(candidate.url.startsWith("https://")) { "Research source must use HTTPS" }
                ArticleExtractor.fetch(candidate.url, connectionFactory, pageTimeoutMillis)
            }.getOrNull()
            if (fetched != null &&
        ...
        persist(session)
        onProgress(DeepResearchProgress("learned", sources.size, diverse.size.coerceAtLeast(1), sources.size, failed))
        return session
    }
    ...
    private fun cancelledSession(query: String, targetSources: Int): ResearchSession = ResearchSession(
        ...
        query = query,
        createdAt = System.currentTimeMillis(),
        requestedSources = targetSources.coerceIn(2, ResearchLimits.MAX_TARGET_SOURCES),
        sources = emptyList(),
        learnedChunks = 0,
