...
import com.codingagent.policy.ConstitutionViolation
...
class MutationCoordinator(
    ...
    private val verificationAfterApply: (ChangeSet) -> VerificationReport = { workspace.verify() }
) {
    ...
    fun approve(id: String, ownerVerified: Boolean, ownerLabel: String): MutationApprovalResult {
        ...
        return try {
            val applied = try {
                ...
                return MutationApprovalResult.Rejected("Approved change could not be applied: ${error.message.orEmpty()}")
            }
            val verification = verificationAfterApply(applied)
            if (!verification.passed) {
                return MutationApprovalResult.Rejected(
                    "Approved change was applied but post-apply verification failed: " +
                        verification.issues.joinToString { "${it.path}:${it.line} ${it.message}" }
                )
            }
            pending.remove(id)
