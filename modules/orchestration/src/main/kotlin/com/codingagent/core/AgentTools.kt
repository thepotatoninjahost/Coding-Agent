...
import com.codingagent.terminal.TerminalSession as ModuleTerminalSession

class AgentTools(private val workspace: ProjectWorkspace) {
    private val terminalSession = ModuleTerminalSession(workspace.projectRoot())
    ...
    fun terminal(
        ...
        return terminalSession.execute(joined, timeoutSeconds, onStdout)
    }
    ...
    fun runTerminal(
        ...
        onStdout: ((String) -> Unit)? = null,
        onStderr: ((String) -> Unit)? = null
    ): TerminalEntry = terminalSession.execute(command, 90, onStdout)
