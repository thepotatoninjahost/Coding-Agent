...
class AgentOrchestrator(
    ...
    fun execute(request: String): List<AgentExecutionEvent> {
        ...
        if (runtime.pendingProposals().isNotEmpty()) {
            events += AgentExecutionEvent.Failed("A pending approval already exists for this workspace")
            return events
        }
    ...
    fun runTerminal(command: String): AgentExecutionEvent.Output {
        ...
        val output = buildString {
            ...
            append(result.result.stdout)
            if (result.result.stderr.isNotBlank()) append(result.result.stderr)
            append("\nexit=").append(result.result.exitCode)
            if (result.result.timedOut) append(" timeout=true")
        }
        return AgentExecutionEvent.Output(output.trimEnd())
    }

    fun research(query: String): List<ResearchHit> = researchProvider.search(query).hits
}
