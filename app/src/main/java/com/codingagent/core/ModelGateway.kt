...
object AgentModelProtocol {
    val DEFAULT_SYSTEM = """Local coding agent. Use tools; never invent file paths or contents.
One tool call per turn. Read a named file before analyzing it.
Changes only stage proposals (dual approval). Verify after edits.
Prefer evidence over guesses. Persist until the goal is done.
    ...
    fun toolsForIntent(intent: TaskIntent): List<ModelToolDefinition> {
        ...
        val names = when (intent) {
            TaskIntent.INSPECT, TaskIntent.EXPLAIN, TaskIntent.UNKNOWN ->
                listOf("list_files", "read_file", "search_project")
            TaskIntent.CHANGE, TaskIntent.CREATE, TaskIntent.REFACTOR, TaskIntent.DEBUG ->
                listOf("list_files", "read_file", "search_project", "replace_text", "create_file", "approve_change", "reject_change", "verify")
            TaskIntent.TEST ->
                listOf("list_files", "read_file", "run_command", "verify")
        }
