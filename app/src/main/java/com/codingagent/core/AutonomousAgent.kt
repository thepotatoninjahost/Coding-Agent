...
class AutonomousAgent(
            ...
            when (response) {
                ...
                is ModelResponse.Text -> {
                    ...
                    if (missing != null) {
                        ...
                        transcript += ModelMessage("assistant", response.content.take(1_200))
                ...
                is ModelResponse.ToolCall -> {
                    ...
                    if (signature == lastToolSignature) {
                        ...
                        if (identicalRepeats >= config.maxIdenticalToolRepeats) {
                            ...
                            // No usable evidence: one nudge toward a different tool, then stop if still stuck.
                            transcript += ModelMessage(
                                "user",
    ...
    private fun isAgentMetaQuestion(t: String): Boolean {
        ...
        val aboutAgent = Regex(
            """\b(why|what)\b.*\b(abort|aborted|stop|stopped|fail|failed|repeat|repeated|loop|tool)\b"""
        )
