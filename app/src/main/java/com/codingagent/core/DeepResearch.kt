...
class DurableDeepResearchProvider(
    ...
    private val maxSourceFetches: Int = 80
    ...
    override fun deepResearch(query: String, targetSources: Int, mode: ResearchMode, onProgress: (DeepResearchProgress) -> Unit): ResearchSession {
        ...
        val effectiveTarget = targetSources.coerceAtMost(24)
        val lanes = QueryLanes.expand(normalized, mode)
        val candidates = lanes.flatMap { lane ->
            val first = searchProvider.search(lane.query, 12).hits
        ...
        if (candidates.size < effectiveTarget) {
            val fallbackQueries = listOf(
                ...
                "$normalized tutorial example",
                "$normalized github",
                "$normalized stackoverflow",
                "$normalized best practices"
            ) + lanes.take(3).flatMap { lane ->
                listOf("${lane.query} reference", "${lane.query} implementation")
            }
            val fallback = fallbackQueries.flatMap { q ->
                searchProvider.search(q, 10).hits.map { hit ->
                    Candidate(hit.title, hit.url, hit.excerpt, lanes.firstOrNull { lane -> q.contains(lane.query.take(24)) }?.name ?: "discovery")
                }
            }.dedupe()
        ...
        if (candidateCount == 0) {
            throw IllegalStateException(
                "No distinct research candidates were found. Network may be blocked, or every search backend returned empty. Try a more specific query or check device internet."
            )
        }
        ...
        futures.forEach { future ->
            ...
            completed += 1
            if (source.error == null && source.content.isNotBlank() && source.wordCount >= 40) {
                sources += source
            } else {
                if (source.content.isNotBlank() && sources.none { it.url == source.url }) {
                    sources += source.copy(error = null)
                } else {
                    failed += 1
                }
            }
        ...
        if (sources.isEmpty()) {
            throw IllegalStateException(
                "No sources could be read. Search found $candidateCount candidates, but every article fetch failed (HTTP blocks, timeouts, or empty bodies)."
            )
        }
        val minimumUsableSources = 2
        if (sources.size < minimumUsableSources) {
            throw IllegalStateException(
                "Only ${sources.size} usable source(s) were learned; at least $minimumUsableSources are required. $failed candidates failed during fetch."
            )
        }
        ...
        val session = ResearchSession(
            ...
            effectiveTarget,
            sources,
            sources.sumOf { source -> chunk(source.content).size },
            errors,
            mode.name.lowercase()
        )
        persist(session)
    ...
    private fun fetch(candidate: Candidate): ResearchSource {
        ...
        return try {
            ...
            connection.setRequestProperty("Accept", "text/html,application/xhtml+xml,text/plain;q=0.9,*/*;q=0.8")
