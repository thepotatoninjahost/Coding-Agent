...
class ProjectWorkspace(private val root: File) {
    ...
    fun verify(): VerificationReport {
        ...
        for (metadata in indexer.index(root)) {
            val file = root.resolve(metadata.path)
            file.readLines().forEachIndexed { index, line ->
                val marker = when {
                    ...
                    line.contains("IMPLEMENT_ME") -> "IMPLEMENT_ME marker remains"
                    line.contains(listOf("throw", "Not" + "ImplementedError").joinToString(" ")) -> "unimplemented code remains"
                    else -> null
                }
                if (marker != null) issues += VerificationIssue(metadata.path, index + 1, marker)
            }
        }
    ...
    fun verifyProposal(changeSet: ChangeSet): VerificationReport {
        ...
        changeSet.changes.forEach { record ->
            ...
            record.after.lineSequence().forEachIndexed { index, line ->
                if (Regex("\\b($TODO_MARKER|$FIXME_MARKER)\\b", RegexOption.IGNORE_CASE).containsMatchIn(line) || line.contains("IMPLEMENT_ME")) {
