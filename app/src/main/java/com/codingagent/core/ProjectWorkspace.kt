...
private const val IMPLEMENT_ME_MARKER = "IMPLEMENT" + "_ME"
...
class ProjectWorkspace(private val root: File) {
    ...
    fun verify(): VerificationReport {
        ...
        for (metadata in indexer.index(root)) {
            ...
            // Skip unit-test sources: fixtures/assertions intentionally mention marker strings.
            ...
            file.readLines().forEachIndexed { index, line ->
                val marker = when {
                    ...
                    line.contains(IMPLEMENT_ME_MARKER) -> "$IMPLEMENT_ME_MARKER marker remains"
    ...
    fun verifyProposal(changeSet: ChangeSet): VerificationReport {
        ...
        changeSet.changes.forEach { record ->
            ...
            record.after.lineSequence().forEachIndexed { index, line ->
                if (Regex("\\b($TODO_MARKER|$FIXME_MARKER)\\b", RegexOption.IGNORE_CASE).containsMatchIn(line) || line.contains(IMPLEMENT_ME_MARKER)) {
...
class CommandRunner(private val directory: File) {
    fun run(command: List<String>, timeoutSeconds: Long): CommandResult {
        ...
        return try {
            ...
            val outThread = Thread { process.inputStream.use { input -> readLimited(input, stdout) } }
            val errThread = Thread { process.errorStream.use { input -> readLimited(input, stderr) } }
            outThread.start()
            errThread.start()
            ...
            CommandResult(command.joinToString(" "), if (completed) process.exitValue() else -1, stdout.toString().trimEnd('\n'), stderr.toString().trimEnd('\n'), !completed)
        } catch (error: Exception) {
            CommandResult(command.joinToString(" "), -1, "", error.message.orEmpty(), false)
        }
    }

    private fun readLimited(input: java.io.InputStream, output: StringBuilder) {
        val buffer = ByteArray(8 * 1024)
        while (true) {
            val count = input.read(buffer)
            if (count < 0) break
            appendLimited(output, String(buffer, 0, count, Charsets.UTF_8))
        }
    }
