...
class NexaLocalModelGateway(
    ...
    override fun stream(request: ModelRequest, onDelta: (String) -> Unit): ModelResponse = synchronized(lock) {
        try {
            runBlocking {
                ...
                if (streamError != null) {
                    return@runBlocking ModelFailure("Nexa local inference failed: $streamError")
                }
                if (output.isBlank()) {
                    sessionDirty = true
                    ModelFailure(
    ...
    private suspend fun prepareSession(): String? {
        val stopMsg = runCatching { wrapper.stopStream() }.fold(
            onSuccess = { "stop=ok" },
            onFailure = { e ->
                val message = e.message.orEmpty().ifBlank { e.javaClass.simpleName }
                Log.e(TAG, "Nexa stopStream failed: $message", e)
                "stopFail=$message"
            }
        )
        ...
        val resetMsg = runCatching { wrapper.reset() }.fold(
            ...
            onFailure = { e ->
                val message = e.message.orEmpty().ifBlank { e.javaClass.simpleName }
                Log.e(TAG, "Nexa reset failed: $message", e)
                "resetFail=$message"
            }
        )
    ...
    fun close() {
        synchronized(lock) {
            runCatching {
                runBlocking {
                    runCatching { wrapper.stopStream() }.onFailure { Log.e(TAG, "Nexa close stopStream failed", it) }
                    runCatching { wrapper.reset() }.onFailure { Log.e(TAG, "Nexa close reset failed", it) }
                }
            }.onFailure { Log.e(TAG, "Nexa session cleanup failed", it) }
            runCatching { wrapper.close() }.onFailure { Log.e(TAG, "Nexa wrapper close failed", it) }
        }
    }
