...
class RemoteHttpGateway(
    ...
    /**
     * Parse OpenAI-compatible SSE (`data: {...}`) or a single JSON completion body.
     * Tool-call argument fragments across deltas are concatenated in order.
     */
    private fun parseStreamBody(raw: String, onDelta: (String) -> Unit): ModelResponse {
        val trimmed = raw.trim()
        if (trimmed.isEmpty()) return ModelResponse.Failure("Model returned an empty response")
        if (trimmed.startsWith("{")) return parseCompletionBody(trimmed)

        data class ToolAcc(var id: String? = null, var name: String = "", val arguments: StringBuilder = StringBuilder())
        ...
        for (line in trimmed.lineSequence()) {
            ...
            val contentPiece = when (val c = delta.opt("content")) {
        ...
        if (firstTool != null) {
            ...
            return ModelResponse.ToolCall(firstTool.name, args, content.toString(), firstTool.id)
        }
