...
class AutonomousAgent(
    ...
    override fun execute(request: String): AgentRuntimeResult {
        ...
        return when (val t = events.lastOrNull()) {
            is AutonomousAgentEvent.ApprovalRequired ->
                AgentRuntimeResult.NeedsApproval(t.task, "Review proposal ${t.proposal.id} and confirm twice before applying any code change.", t.proposal.id)
    ...
    fun run(request: String, onEvent: (AutonomousAgentEvent) -> Unit = {}): List<AutonomousAgentEvent> {
        ...
        if (ResearchGate.shouldAutoResearch(normalized, intake)) {
            ...
            if (session != null && session.sources.isNotEmpty()) {
                ...
                emit(AutonomousAgentEvent.Phase("RESEARCH", "No usable sources"))
            }
        } else {
    ...
    private fun approvalTask(id: String, request: String, plan: AgentPlan, proposal: PendingChangeProposal): AgentTask =
        AgentTask(id, request, "waiting-approval", plan, proposal.changeSet.changes, proposal.verification, listOf("${Instant.now()}: proposal ${proposal.id} staged"), "Review proposal ${proposal.id} and confirm twice before applying any code change")
}
